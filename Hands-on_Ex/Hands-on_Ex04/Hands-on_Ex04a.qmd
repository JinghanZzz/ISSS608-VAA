...
funnel_plot(
  ...
  # 加在这里！ 👇
  title = "COVID-19 Fatality Rate by Sub-district in Jakarta",
  x_label = "Number of Positive Cases",
  y_label = "Fatality Rate (Death / Positive)"
)
...
p <- ggplot(df, aes(x = Positive, y = rate)) +
  ...
  ggtitle("Cumulative Fatality Rate \n by Cumulative Number of COVID-19 Cases") +
...
![](images/clipboard-165433941.png)
...
::: callout-note
loge(BF01)=-31.45 这个是bayes Factor 去查表 越负越支持H1(备择假设)

posterior difference就是后验-先验的差距，就是比60分高7.16

ci是posterior difference的ci，就是95%的可能他们俩的差距在5.54\~8.75之间

最后那个0.71不懂

蓝色竖线是后验最大可能的均值（Maximum A Posteriori）
:::
...
![](images/clipboard-388353140.png)
...
![](images/clipboard-2210485552.png)
...
#这个算出来0在95%CI里面，也就是说差异极大可能是不显著的
#检验是否有差异用双侧检验，检验A>B?用单侧（A-B>0?; A-B<=0)
```
...
![](images/clipboard-410195674.png)

![](images/clipboard-1206665820.png)
...
ggbarstats(exam1, 
           x = MATHS_bins, 
           y = GENDER)
#这个图其实就是在做卡方检验看男女在math分数有没有显著区别 算出来没有
```
...
```{r}
#这个point estimate就是对应interval estimate。 point estimate出来是必须μ，p，σ^2这种单一值，然后interval就是一个置信区间
```

::: callout-important
-   Don’t confuse the uncertainty of a point estimate with the variation in the sample

-   **Sample variation** refers to how spread out individual data points are (measured by standard deviation).

-   **Point estimate uncertainty** refers to how precise your estimate of the population parameter is (measured by standard error or confidence interval).
:::
...
for mathmatical explanation, refer to Slide 20 of Lesson 4
:::
